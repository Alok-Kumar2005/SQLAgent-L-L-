...
class PrepareVectorDB:
    ...
    Attributes:
        ...
        collection_name (str): The name of the collection to be used (less relevant for FAISS but kept for compatibility).
    ...
    def __init__(self,
        ...
        self.collection_name = collection_name  # Less relevant for FAISS but kept for compatibility
    ...
    def run(self):
        ...
        if not os.path.exists(here(self.vectordb_dir)):
            ...
            vectordb.save_local(folder_path=str(here(self.vectordb_dir)))
            ...
            # Optionally, we could load the existing index to show vector count
            # embeddings = GoogleGenerativeAIEmbeddings(model=self.embedding_model)
            # vectordb = FAISS.load_local(folder_path=str(here(self.vectordb_dir)), embeddings=embeddings)
            # print("Number of vectors in existing vectordb:", len(vectordb.index_to_docstore_id), "\n\n")
